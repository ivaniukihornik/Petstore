...
def test_finding_of_existing_pet(create_pet_api, pc_create_new_pet):
    api = create_pet_api
    pet_id = pc_create_new_pet.id
    response = api.get_pet(pet_id)
    assert response.status_code == HTTPStatus.OK, 'Wrong response status code'
...
def test_finding_of_unexisting_pet(create_pet_api, pc_delete_pet_if_exists):
    ...
    response = api.get_pet(pet_id)
    assert response.status_code == HTTPStatus.NOT_FOUND, 'Wrong response status code'
...
def test_finding_of_pet_with_invalid_id(create_pet_api, pet_id):
    ...
    assert response.status_code == HTTPStatus.BAD_REQUEST, 'Wrong response status code'
...
def test_pet_creating_with_invalid_data(create_pet_api, pc_delete_pet_if_exists, pc_generate_pet_object, field, value):
    ...
    assert response.status_code == HTTPStatus.METHOD_NOT_ALLOWED, 'Wrong response status code'
...
def test_pet_creating_with_unsupported_content_type_header(create_pet_api, pc_generate_pet_object,
    ...
    assert response.status_code == HTTPStatus.UNSUPPORTED_MEDIA_TYPE, 'Wrong response status code'
